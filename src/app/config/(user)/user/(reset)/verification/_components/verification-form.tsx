...
export default function VerificationForm() {
  ...
  const handleOnSubmit = async () => {
    ...
    if (validityCheck) {
      const verifiedState = await AuthService.postUserCheckProcessByLoginUser(formData);
  ...
  return (
    <>
      ...
      <Modal isOpen={externalList.has('userCheck')} setIsOpen={handleModalClose}>
        <Modal.Content colorTheme="alert" title="본인 확인 중 에러 발생" desc={modalMsg ?? ''} />
