...
export default function Edit() {
  ...
  const handleOnSubmit = async () => {
    ...
    if (validityCheck) {
      const updateProfileState = await AuthService.updateProfileProcess(formData.username);
    ...
    <>
      ...
      <Form onSubmit={handleOnSubmit} formData={formData} setFormData={setFormData} styles="px-4">
        <div className="flex flex-col mb-3">
          ...
          <Form.ErrorText>{changeUsernameErrorMsg(formData.username)}</Form.ErrorText>
        </div>
      </Form>
      <footer className="mt-auto mb-0 p-4 fixed bottom-0 max-w-[379px] w-full bg-white">
        <Form.Button
          type="submit"
          colorTheme={checkUsernameDataTypeCheck(formData) ? 'primary' : 'invalidReverse'}
        >
          내 정보 저장
        </Form.Button>
      </footer>
      <Modal isOpen={externalList.has('editProfile')} setIsOpen={handleModalClose}>
        <Modal.Content colorTheme="alert" title="프로필 수정 중 에러 발생" desc={modalMsg ?? ''} />
