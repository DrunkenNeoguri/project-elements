...
export default function TemplateSection({ handlePrevStep, travelData }: PropType) {
  const { modalMsg, setModalMsg, externalList, handleExternalList, user, router } =
    useTemplateSection();
  ...
  const handleMoveToMakeList = async (useTemplate: boolean) => {
    ...
    if (
      ...
      (travelData.travelType !== 'domestic' && travelData.travelType !== 'foreign')
    ) {
      handleExternalList('travelCreate');
      return setModalMsg(
    ...
    if (createListState === 'OK') {
      return router.push(`/element/edit?id=${travelData.id}`);
    }
  };
  ...
  return (
    <section className="flex flex-col items-start h-full box-border">
      ...
      <Modal isOpen={externalList.has('travelCreate')} setIsOpen={handleCloseModal}>
        <Modal.Content colorTheme="alert" title="여행 정보 등록 불가" desc={modalMsg ?? ''} />
