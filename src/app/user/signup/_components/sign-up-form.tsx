...
export default function SignUpForm() {
  ...
  return (
    <>
      <Form onSubmit={handleSubmit} formData={signUpData} setFormData={setSignUpData} styles="px-4">
        ...
        <div className="flex flex-col mb-3">
          ...
          <Form.ErrorText>{changePasswordErrorMsg(signUpData.password)}</Form.ErrorText>
        </div>
        ...
        <div className="flex flex-col mb-3">
          ...
          <Form.ErrorText>
            {changeConfirmPasswordErrorMsg(signUpData.password, signUpData.confirmPassword)}
          </Form.ErrorText>
        </div>
        ...
        <div className="flex flex-col mb-3">
          ...
          <Form.ErrorText>{changeUsernameErrorMsg(signUpData.username)}</Form.ErrorText>
        </div>

        <div className="flex flex-col gap-3 mt-3">
          <Form.Button
            disabled={!checkSignUpDataTypeCheck(signUpData)}
            colorTheme={checkSignUpDataTypeCheck(signUpData) ? 'primary' : 'invalidReverse'}
            type="submit"
          >
            {checkSignUpDataTypeCheck(signUpData) ? '다음 단계로' : '내용을 모두 기입해주세요.'}
          </Form.Button>

          <Form.Button colorTheme="invalid" type="button" onClick={handleMoveToLoginPage}>
            돌아가기
          </Form.Button>
        </div>
      </Form>
      <Modal isOpen={externalList.has('signup')} setIsOpen={handleCloseModal}>
        <Modal.Content colorTheme="alert" title="회원가입 중 에러 발생" desc={modalMsg ?? ''} />
